...
describe('Momo', () => {
  ...
  describe('with long ping interval', () => {
    beforeEach(async () => {
      ...
      connection = await Connection.create({ url: mongo.getUri() }, 0, mongoSchedule.id(), scheduleName);
    ...
    describe('single interval job', () => {
      ...
      beforeEach(() => {
        ...
        intervalJob = createTestIntervalJob(jobHandler, { interval: 300, firstRunAfter: 0 });
      });
      ...
      it('executes a job periodically', async () => {
        ...
        await waitFor(() => expect(jobHandler.count).toBe(2), 400);
      });

      it('executes job periodically after firstRunAfter', async () => {
        await mongoSchedule.define({
          ...intervalJob,
          schedule: { ...intervalJob.schedule, firstRunAfter: 300 },
        });

        await mongoSchedule.start();

        expect(jobHandler.count).toBe(0);
        await waitFor(() => expect(jobHandler.count).toBe(1), 400);
        await waitFor(() => expect(jobHandler.count).toBe(2), 300);
      });

      it('executes job periodically with human-readable firstRunAfter', async () => {
        await mongoSchedule.define({
          ...intervalJob,
          schedule: { ...intervalJob.schedule, firstRunAfter: '1 second' },
        });
      ...
      it('saves executionInfo in mongo', async () => {
        ...
        expect(executionInfo!.lastResult).toEqual({
    ...
    describe('several jobs', () => {
      ...
      beforeEach(() => {
        ...
        job1 = createTestIntervalJob(jobHandler1, { interval: 300, firstRunAfter: 0 });
        job2 = createTestIntervalJob(jobHandler2, { interval: 300, firstRunAfter: 0 });
      });
    ...
    describe('long running interval jobs', () => {
      ...
      beforeEach(() => {
        ...
        job = createTestIntervalJob(jobHandler, { interval: 300, firstRunAfter: 0 });
      });
      ...
      it('executes a long running job', async () => {
        ...
        await mongoSchedule.start();
        await waitFor(() => expect(jobHandler.count).toBe(1), jobHandler.duration + 300);
      ...
      it('respects maxRunning', async () => {
        ...
        await sleep(700);
        const runningAfter700ms = await schedulesRepository.countRunningExecutions(job.name);
        expect(runningAfter700ms).toBe(2);

        await sleep(400);
        const runningAfter1000ms = await schedulesRepository.countRunningExecutions(job.name);
        expect(runningAfter1000ms).toBe(2);
      ...
      it('starts job twice if maxRunning is set to 0 (no max, default)', async () => {
        await mongoSchedule.define(job);
      ...
      it('stopped job does not decrease execution count', async () => {
        ...
        await sleep(jobHandler.duration + 300);
        expect(jobHandler.count).toBe(1);
    ...
    describe('concurrent job', () => {
      ...
      beforeEach(() => {
        ...
        job = { ...createTestIntervalJob(jobHandler, { interval: 300, firstRunAfter: 0 }), concurrency: 3 };
      });
    ...
    describe('job parameters', () => {
      ...
      beforeEach(() => {
        intervalJob = {
          handler: jest.fn(),
          name: 'test',
          schedule: { interval: 300, firstRunAfter: 0 },
          concurrency: 3,
        };
      });
  ...
  describe('with short ping interval', () => {
    ...
    beforeEach(async () => {
      mongoSchedule = await MongoSchedule.connect({ pingIntervalMs: 200, scheduleName, url: mongo.getUri() });
    ...
    it('respects maxRunning if job runs longer than ping interval', async () => {
      await mongoSchedule.define({ ...job, maxRunning: 1 });

      await mongoSchedule.start();

      await sleep(200);
      const runningAfter200ms = await schedulesRepository.countRunningExecutions(job.name);
      expect(runningAfter200ms).toBe(1);

      await sleep(200);
      const runningAfter400ms = await schedulesRepository.countRunningExecutions(job.name);
      expect(runningAfter400ms).toBe(1);

      // verify that schedule pinged during the test
      const schedule = await schedulesRepository.findOne({ name: scheduleName });
      expect(schedule?.lastAlive).toBeGreaterThan(Date.now() - 300);
    });
  });
});
