...
describe('Momo', () => {
  ...
  describe('with long ping interval', () => {
    ...
    describe('single interval job', () => {
      ...
      it('does not update interval of started job from mongo', async () => {
        ...
        expect(schedule).toEqual({
          ...
          firstRunAfter: updatedSchedule.firstRunAfter,
        });
    ...
    describe('single cron job', () => {
      ...
      beforeEach(() => {
        jobHandler = createTestJobHandler();
        cronJob = createTestCronJob(jobHandler, {
          cronSchedule: '*/1 * * * * *',
        });
      });
  ...
  describe('with short ping interval', () => {
    ...
    it('respects maxRunning if job runs longer than ping interval', async () => {
      ...
      await sleep(200);
      const runningAfter200ms = await schedulesRepository.countRunningExecutions(
        job.name,
      );
      expect(runningAfter200ms).toBe(1);

      await sleep(200);
      const runningAfter400ms = await schedulesRepository.countRunningExecutions(
        job.name,
      );
