  ...
  return {
    Connection: {
      create: async (_options: MomoOptions) => {
        return {
          getJobRepository: () => jobRepositoryMock.instance,
...
describe('Schedule', () => {
  ...
  const entityWithId = {
    _id: new ObjectId(),
    ...toJobDefinition(tryToIntervalJob(momoJob)._unsafeUnwrap()),
  };
  ...
  beforeEach(async () => {
    ...
    jobRepositoryMock.stubs.find.mockResolvedValue([]);
  ...
  it('emits logs', async () => {
    ...
    mongoSchedule.on('debug', (event: MomoEvent) =>
      caughtEvents.push(event.message),
    );
    mongoSchedule.on('error', (event: MomoEvent) =>
      caughtErrors.push(event.message),
    );
  ...
  describe('run', () => {
    it('runs a job once', async () => {
      ...
      jobRepositoryMock.stubs.findOne.mockResolvedValue(entityWithId);
