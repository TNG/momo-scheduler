...
describe('SchedulesRepository', () => {
  ...
  describe('isActiveSchedule', () => {
    it('sets single schedule as active', async () => {
      await schedulesRepository.setActiveSchedule(scheduleName);
      const schedule = await schedulesRepository.findOne({ name: scheduleName });

      expect(schedule!.scheduleId).toEqual(scheduleId);
    });

    it('single schedule is active', async () => {
      await schedulesRepository.setActiveSchedule(scheduleName);

      const active = await schedulesRepository.getActiveSchedule(scheduleName);

      expect(active).toEqual(scheduleId);
    });
