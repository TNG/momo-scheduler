...
export const momoError = {
  ...
  invalidConcurrency: new Error(
    'concurrency must be at least 1 but not greater than maxRunning',
  ),
  invalidMaxRunning: new Error('maxRunning must be at least 0'),
  invalidInterval: new Error(
    `interval must be greater than 0 and less than ${maxNodeTimeoutDelay}`,
  ),
  invalidTimeout: new Error(
    `timeout must be greater than 0 and less than ${maxJobTimeout}`,
  ),
