...
minutes" or "run that job according to this cron schedule".
...
| withSchedule     | `string`,`number` | false     |               | Either this setter or `withCronSchedule()` must be called. Specifies the time interval at which the job is started. Time intervals in human-readable formats ( like '1 minute', 'ten days' or 'twenty-one days and 2 hours' ) are accepted. Check documentation of [ human-interval ](https://www.npmjs.com/package/human-interval) library for details.If the job never ran before, the job will run after `firstRunAfter` milliseconds for the first time. |
