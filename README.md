...
| delay       | `number`   | true     |         | If set AND the job was never run before, first job execution will be started after `delay` milliseconds. If set to `0`, the job will be run immediately after the start. If omitted, the job will be run after the `interval` has elapsed.                                                                     |
