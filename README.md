...
| firstRunAfter | `number`   | true     |         | If set AND the job was never run before, the job will be run after `firstRunAfter` milliseconds for the first time. If set to `0`, the job will be run immediately once the MongoSchedule is started. If omitted, the job will be run after the `interval` has elapsed.                                        |
